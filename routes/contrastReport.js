...
function buildReport(colors) {
  const byRole = {};
  colors.forEach((c) => { byRole[c.role] = c.hex; });
  ...
  AUDIT_PAIRS.forEach(([fgRole, bgRole]) => {
    if (byRole[fgRole] && byRole[bgRole]) {
      ...
      pairs.push({
        fgRole, bgRole,
        fgHex: byRole[fgRole], bgHex: byRole[bgRole],
        ratio: r.ratio, AA: r.AA, AAA: r.AAA,
      });
    }
  });
...
router.post('/', async (req, res) => {
  ...
  if (!ObjectId.isValid(paletteId)) return res.status(400).json({ error: 'bad paletteId' });
