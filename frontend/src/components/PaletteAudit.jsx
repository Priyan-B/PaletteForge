...
import { listPalettes, listReports, generateReport, deleteReport } from '../../api/contrastApi';
          ...
          <table className="pa__table">
            ...
            <tbody>
              {active.pairs.map((p, i) => (
                <tr key={i}>
                  <td>
                    <span className="pa__chip" style={{ background: p.fgHex }} />
                    <span className="pa__chip" style={{ background: p.bgHex }} />
                    {p.fgRole} on {p.bgRole}
                  </td>
                  <td>{p.ratio}:1</td>
                  <td className={p.AA ? 'pass' : 'fail'}>{p.AA ? 'Pass' : 'Fail'}</td>
                  <td className={p.AAA ? 'pass' : 'fail'}>{p.AAA ? 'Pass' : 'Fail'}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <ul className="pa__list">
          {reports.map((r) => (
            <li key={r._id} className="pa__item">
              <button type="button" className="pa__link" onClick={() => setActive(r)}>
                {r.paletteName}
              </button>
              <span className="pa__muted">
                {r.summary.aaFails}/{r.summary.total} fail AA
              </span>
              <button type="button" className="pa__ghost" onClick={() => handleDelete(r._id)}>
                Delete
              </button>
            </li>
          ))}
